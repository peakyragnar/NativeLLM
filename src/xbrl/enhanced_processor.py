...
def process_filing_ixbrl(filing_metadata, force_download=False):
    """
    ...
    print(f"Processing {ticker} {filing_type} using iXBRL approach")
    
    # If we have a document_url from the enhanced URL detection, use it
    if "document_url" in filing_metadata:
        print(f"Using pre-discovered document URL: {filing_metadata['document_url']}")
    ...
    if "error" in download_result:
        print(f"Error downloading iXBRL document: {download_result['error']}")
    ...
    if "error" in extracted_data:
        print(f"Error extracting iXBRL data: {extracted_data['error']}")
...
def process_filing_xbrl(filing_metadata, force_download=False):
    """
    ...
    if "error" in download_result:
        print(f"Error downloading XBRL: {download_result['error']}")
        return download_result
    ...
    if "error" in parsed_result:
        print(f"Error parsing XBRL: {parsed_result['error']}")
...
def process_company_filing(filing_metadata, force_download=False):
    """
    ...
    print(f"[{ticker} {filing_type}] Determined preferred format: {preferred_format}")
    
    # Save original URLs for logging/debugging
    instance_url = filing_metadata.get("instance_url", "None")
    print(f"[{ticker} {filing_type}] Instance URL: {instance_url}")
    ...
    if preferred_format == "ixbrl":
        ...
        print(f"[{ticker} {filing_type}] Attempting iXBRL processing first (newer format)")
        result = process_filing_ixbrl(filing_metadata, force_download)
        
        # If iXBRL processing fails, fall back to traditional XBRL
        if "error" in result:
            error = result.get("error", "Unknown error")
            print(f"[{ticker} {filing_type}] iXBRL processing failed: {error}")
            print(f"[{ticker} {filing_type}] Falling back to traditional XBRL")
            result = process_filing_xbrl(filing_metadata, force_download)
        ...
        print(f"[{ticker} {filing_type}] Attempting iXBRL processing first (likely format)")
        ...
        if "error" in result:
            error = result.get("error", "Unknown error")
            print(f"[{ticker} {filing_type}] iXBRL processing failed: {error}")
            print(f"[{ticker} {filing_type}] Trying traditional XBRL")
            result = process_filing_xbrl(filing_metadata, force_download)
    
    elif preferred_format == "hybrid":
        # For ambiguous cases, try both formats
        print(f"[{ticker} {filing_type}] Using hybrid approach - trying both formats")
        ...
        try:
            ...
            if filing_year >= 2020:
                print(f"[{ticker} {filing_type}] Post-2020 filing, trying iXBRL first")
                result = process_filing_ixbrl(filing_metadata, force_download)
                ...
                if "error" in result:
                    error = result.get("error", "Unknown error")
                    print(f"[{ticker} {filing_type}] Traditional XBRL processing failed: {error}")
                    print(f"[{ticker} {filing_type}] Trying iXBRL format")
                    result = process_filing_ixbrl(filing_metadata, force_download)
        except:
            # If date parsing fails, try both formats
            print(f"[{ticker} {filing_type}] Could not determine filing year, trying XBRL first")
            result = process_filing_xbrl(filing_metadata, force_download)
        ...
        print(f"[{ticker} {filing_type}] Using default format - trying XBRL first")
        result = process_filing_xbrl(filing_metadata, force_download)
    ...
    if "error" in result:
        print(f"[{ticker} {filing_type}] Final result: ERROR - {result.get('error')}")
    else:
        print(f"[{ticker} {filing_type}] Final result: SUCCESS - Using {result.get('processing_path', 'unknown')} approach")
        if "fact_count" in result:
            print(f"[{ticker} {filing_type}] Extracted {result.get('fact_count', 0)} facts")
            
    return result

def process_and_format_filing(filing_metadata, force_download=False):
    """
    ...
    """
    print(f"Processing filing: {filing_metadata.get('ticker')} {filing_metadata.get('filing_type')}")
    
    # Process the filing using the enhanced processor
    result = process_company_filing(filing_metadata, force_download)
    
    if "error" in result:
        print(f"Error processing filing: {result['error']}")
        return result
    
    print(f"Successfully processed filing using {result.get('processing_path', 'unknown')} approach")
    print(f"Source file: {result.get('file_path')}")
