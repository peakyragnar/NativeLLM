#!/usr/bin/env python3
"""
Test Clean JSON Format

This script tests the clean JSON output format for SEC filings.
"""
...
# Import the clean JSON formatter
from src2.formatter.clean_json_formatter import clean_json_formatter

def main():
    """
    Main function to test clean JSON output format
    """
    parser = argparse.ArgumentParser(description="Test clean JSON output format for SEC filings")
    ...
    # Generate clean JSON format
    json_data = clean_json_formatter.generate_json_format(args.html_path, metadata)

    # Save clean JSON format
    save_result = clean_json_formatter.save_json_format(json_data, args.output)

    if save_result.get("success", False):
        logging.info(f"Successfully saved clean JSON format to {args.output}")
        
        # Print JSON structure summary
        print(f"\nClean JSON file size: {os.path.getsize(args.output) / 1024:.2f} KB")
        ...
        if os.path.exists(original_path):
            ...
            print(f"  - Clean JSON: {new_size / 1024:.2f} KB")
            print(f"  - Difference: {size_diff / 1024:.2f} KB ({size_diff_percent:.2f}%)")
    else:
        logging.error(f"Error saving clean JSON format: {save_result.get('error', 'Unknown error')}")
