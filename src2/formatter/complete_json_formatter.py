#!/usr/bin/env python3
"""
Complete JSON Formatter Module

Provides a complete JSON formatter that extracts both narrative sections and financial data
...
# Import with absolute paths to avoid import errors
...
class CompleteJSONFormatter:
    """
    Complete JSON formatter that extracts both narrative sections and financial data
    directly from HTML/XBRL, similar to the .txt implementation.
    """

    def __init__(self):
        """Initialize the complete JSON formatter"""
    ...
    def generate_json_format(self, html_path: str, filing_metadata: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if extract_result.get("success", False):
                ...
                logging.error(f"Error extracting document sections: {extract_result.get('error', 'Unknown error')}")
            ...
            logging.info(f"Extracting contexts from {html_path}")
            contexts = extract_contexts_from_html(html_path)

            # Store contexts
            json_data["xbrl_data"]["contexts"] = contexts

            # Organize financial statements using the FinancialStatementOrganizer
            logging.info("Organizing financial statements")
            organizer = FinancialStatementOrganizer()

            # Organize facts by statement type
            facts_by_statement = organizer.organize_facts(xbrl_facts)
            ...
            logging.error(f"Error generating JSON format: {str(e)}")
...
complete_json_formatter = CompleteJSONFormatter()
