...
class ArelleRenderer:
    ...
    def _install_arelle(self):
        ...
        try:
            ...
            # First uninstall any existing installation to ensure a clean install
            subprocess.run(
                [sys.executable, "-m", "pip", "uninstall", "-y", "arelle"],
                check=False,
                capture_output=True,
                text=True
            )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    logging.info(f"Successfully imported Arelle module")
                    self.arelle_path = "module"  # Mark as module-based
                    return True
                except ImportError as e:
                    logging.warning(f"Could not import Arelle module: {str(e)}")
    ...
    def render_ixbrl(self, input_file, output_format="html", output_file=None):
        ...
        if self.arelle_path == "module":
            ...
            try:
                ...
                try:
                    import arelle
                    arelle_path = os.path.dirname(arelle.__file__)
                    cmdline_path = os.path.join(arelle_path, "CntlrCmdLine.py")

                    if os.path.exists(cmdline_path):
                        logging.info(f"Found Arelle CntlrCmdLine.py at: {cmdline_path}")
                        cmd_base = [sys.executable, cmdline_path]
                except ImportError:
                    logging.warning("Could not import arelle module to find path")
            except Exception as e:
                logging.warning(f"Error finding Arelle module path: {str(e)}")
