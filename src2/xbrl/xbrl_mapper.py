...
class XBRLMapper:
    ...
    def _output_hierarchy(self, concept, parent_to_children, output, indent=0, max_depth=5, current_depth=0, visited=None):
        ...
        Args:
            ...
            visited: Set of visited concepts to prevent cycles
        """
        ...
        if concept in visited:
            output.append(f"{' ' * indent}{concept} [CYCLE DETECTED]")
            return

        # Check recursion depth to prevent stack overflow
        if current_depth >= max_depth:
            output.append(f"{' ' * indent}{concept} [MAX DEPTH REACHED]")
            return

        # Add concept to visited set
        visited.add(concept)
