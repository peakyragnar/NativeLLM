...
def process_filing(filing_metadata, include_html=True, include_xbrl=True, use_enhanced_processor=True):
    ...
    try:
        ...
        if include_html:
            # Special handling for older filings (2022-2023)
            filing_year = 0
            if fiscal_year:
                try:
                    ...
                    if period_end_date and len(period_end_date) >= 4:
                        try:
                            filing_year = int(period_end_date[0:4])
                        except:
                            pass
                
            # Apply special handling for 2022-2023 filings
            if filing_year in [2022, 2023]:
                logging.info(f"Applying special handling for {filing_year} filing")
                
                # Check if we have an index URL
                if "index_url" in filing_metadata:
                    index_url = filing_metadata["index_url"]
                    logging.info(f"Pre-2024 filing, using index_url: {index_url}")
                    
                    # Extract accession number from URL if available
                    accession_number = filing_metadata.get("accession_number", "")
                    if accession_number:
                        # Construct directory URL for the filing
                        dir_parts = index_url.split("/")
                        if len(dir_parts) >= 7:  # Check if URL has enough parts
                            # Typical URL: https://www.sec.gov/Archives/edgar/data/320193/000032019323000006/...
                            base_dir = "/".join(dir_parts[:-1])  # Remove last part
                            
                            # Try different document naming conventions for older years
                            potential_html_names = [
                                f"{ticker.lower()}-{period_end_date.replace('-', '')}.htm",
                                f"{ticker.lower()}{period_end_date.replace('-', '')}.htm",
                                f"{accession_number}-index.htm",
                                "index.htm",
                                f"{ticker.lower()}.htm",
                                f"{ticker.lower()}-10k.htm" if filing_type == "10-K" else f"{ticker.lower()}-10q.htm",
                                f"{ticker.lower()}10k.htm" if filing_type == "10-K" else f"{ticker.lower()}10q.htm",
                            ]
                            
                            # Add these potential URLs to metadata
                            filing_metadata["potential_html_urls"] = [f"{base_dir}/{name}" for name in potential_html_names]
                            logging.info(f"Added {len(filing_metadata['potential_html_urls'])} potential HTML URLs for {filing_year} filing")
